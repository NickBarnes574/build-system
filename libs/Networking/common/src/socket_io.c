...
int send_data(int socket, void * buffer_p)
{
    ...
    {
        exit_code =
            safe_fprintf(stderr, "send_data(): socket %d hung up.\n", socket);
        if (E_SUCCESS != exit_code)
        {
            print_error("send_data(): fprintf() error.");
        }
        goto END;
    }
...
int recv_data(int socket, void * buffer_p)
{
    ...
    {
        exit_code =
            safe_fprintf(stderr, "recv_data(): socket %d hung up.\n", socket);
        if (E_SUCCESS != exit_code)
        {
            print_error("send_data(): fprintf() error.");
        }
...
int recv_all_data(int socket, void * buffer_p, size_t bytes_to_recv)
{
    ...
    while (total_bytes_received < bytes_to_recv)
    {
        ...
        if (0 == byte_result)
        {
            ...
            {
                exit_code =
                    safe_fprintf(stderr, "client [%d] was closed.\n", socket);
                if (E_SUCCESS != exit_code)
                {
                    print_error("send_data(): fprintf() error.");
                }
