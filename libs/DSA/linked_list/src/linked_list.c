...
void * list_pick_random_item(list_t * list)
{
    ...
    int           random_index = 0; // Changed to int
    void *        data         = NULL;
    ...
    // Ensure list->size - 1 fits within the range of an int
    if (list->size > INT_MAX)
    {
        print_error("list_pick_random_item(): List size too large.");
        goto END;
    }

    exit_code = random_number(0, (int)(list->size - 1), &random_index);
    if (E_SUCCESS != exit_code)
    {
        print_error(
