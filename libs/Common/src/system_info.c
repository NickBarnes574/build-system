...
char * get_hostname()
{
    ...
    if (NULL == result)
    {
        print_error("get_hostname(): CMR failure - result");
        result = NULL;
        goto END;
    }
    ...
    {
        exit_code = safe_snprintf(result, BUFFER_SIZE, "%s", hostname);
        if (E_SUCCESS != exit_code)
        {
            print_error("get_hostname(): snprintf() error.");
            free(result);
            result = NULL;
            goto END;
        }
...
char * get_operating_system()
{
    ...
    if (NULL == os_release)
    {
        print_error("get_operating_system(): Unable to open file.");
        goto END;
    }
    ...
    if (E_SUCCESS != exit_code)
    {
        print_error("get_operating_system(): Error closing file.");
        goto END;
    }
...
char * get_cpu_info()
{
    ...
    if (NULL == cpuinfo)
    {
        print_error("get_cpu_info(): Unable to open file.");
        goto END;
    }
    ...
    if (E_SUCCESS != exit_code)
    {
        print_error("get_cpu_info(): Error closing file.");
        goto END;
    }
...
char * get_cpu_architecture()
{
    ...
    if (NULL == result)
    {
        print_error("get_cpu_architecture(): CMR failure - result.");
        goto END;
    }

    exit_code = safe_snprintf(result, BUFFER_SIZE, "%s", buffer.machine);
    if (E_SUCCESS != exit_code)
    {
        print_error("get_cpu_architecture(): snprintf() error.");
        free(result);
        result = NULL;
        goto END;
    }
...
char * get_memory_info()
{
    ...
    if (NULL == meminfo)
    {
        print_error("get_memory_info(): Unable to open file.");
        goto END;
    }
    ...
    if (E_SUCCESS != exit_code)
    {
        print_error("get_memory_info(): Error closing file.");
        goto END;
    }
...
char * get_network_interfaces()
{
    ...
    if (NULL == netdev)
    {
        print_error("get_network_interfaces(): Unable to open file.");
        goto END;
    }
    ...
    if (NULL == result)
    {
        print_error("get_network_interfaces(): Memory allocation failure.");
        goto END;
    }
    ...
    exit_code = safe_snprintf(result, BUFFER_SIZE, "Network Interfaces:\n");
    if (E_SUCCESS != exit_code)
    {
        print_error("get_network_interfaces(): snprintf() error.");
        free(result);
        result = NULL;
        goto END;
    }
    ...
    if (E_SUCCESS != exit_code)
    {
        print_error("get_network_interfaces(): Error closing file.");
        free(result);
        result = NULL;
    }
...
static char * get_os_name(char * buffer)
{
    ...
    if (NULL == buffer)
    {
        print_error("get_os_name(): NULL argument passed.");
        goto END;
    }
    ...
    if (result == NULL)
    {
        print_error("get_operating_system(): CMR failure - result.");
        goto END;
    }
    ...
    if (0 == strncmp(buffer, "PRETTY_NAME=", PRETTY_NAME_LEN))
    {
        ...
        if (name != NULL)
        {
            ...
            {
                *end      = '\0';
                exit_code = safe_snprintf(result, BUFFER_SIZE, "%s", name);
                if (E_SUCCESS != exit_code)
                {
                    print_error("get_os_name(): snprintf() error.");
                    free(result);
                    result = NULL;
                }
                goto END;
...
static char * parse_cpu_model(char * buffer)
{
    ...
    if (NULL == buffer)
    {
        print_error("parse_cpu_model(): NULL argument passed.");
        goto END;
    }
    ...
    if (0 == strncmp(buffer, "model name", MODEL_NAME_LEN))
    {
        ...
        if (NULL == result)
        {
            print_error("parse_cpu_model(): Memory allocation failure.");
            goto END;
        }
        ...
        exit_code =
            safe_snprintf(result, BUFFER_SIZE, "%s", strchr(buffer, ':') + 2);
        if (E_SUCCESS != exit_code)
        {
            print_error("parse_cpu_model(): snprintf() error.");
            goto END;
        }
    }
...
static char * parse_memory_info(char * buffer)
{
    ...
    if (NULL == buffer)
    {
        print_error("parse_memory_info(): NULL argument passed.");
        goto END;
    }
    ...
    if (0 == strncmp(buffer, "MemTotal", 8))
    {
        ...
        if (NULL == result)
        {
            print_error("parse_memory_info(): Memory allocation failure.");
            goto END;
        }
        ...
        exit_code  = safe_snprintf(result, BUFFER_SIZE, "%ld MB", mem_mb);
        if (E_SUCCESS != exit_code)
        {
            print_error("parse_memory_info(): snprintf() error.");
            free(result);
            result = NULL;
            goto END;
        }
